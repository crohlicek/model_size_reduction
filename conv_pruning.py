...
#PCT HERE SPECIFIES THE PROPORTION OF WEIGHTS TO BE REMOVED
#---------------------------------------------------------
# input: weight matrix mtx, percent to remove pct; output: pruned tensor, mask
def magnitude_prune_conv(mtx, pct):
    ...
    for i in range(len(mtx)):
        for j in range(len(mtx[i])):
            for k in range(len(mtx[i][j])):
                for l in range(len(mtx[i][j][k])):
                    if np.abs(mtx[i][j][k][l]) < threshold:
                        mtx[i][j][k][l]=0.0
...
def generate_conv_prune_network(net, percent):
    c1 = net.conv1.weight.data
    c2 = net.conv2.weight.data

    conv1_pruned, conv1_mask = magnitude_prune_conv(c1, percent)
    conv2_pruned, conv2_mask = magnitude_prune_conv(c2, percent)

    net.conv1.weight.data = conv1_pruned
    net.conv1.weight.data = conv1_pruned

    return conv1_mask, conv2_mask
...
def conv_training_with_data_collection(net, data_loaders, num_epochs, patience, conv1_mask, conv2_mask):
    # Loss and optimizer
        ...
        for phase in ['train', 'val']:
            ...
            for i, (images, labels) in enumerate(data_loaders[phase]):
                ...
                if phase == 'train':
                    ...
                    net.conv1.weight.grad.data.mul_(conv1_mask)
                    net.conv2.weight.grad.data.mul_(conv2_mask)
                    #-------------------------------------------
                    # update the weights
                    optimizer.step()
...
def collect_prune_conv_data(net, p_list):
    ...
    for percent in p_list:
        ...
        conv1_mask, conv2_mask = generate_conv_prune_network(net, percent)
        ...
        a,b,c,d,e = conv_training_with_data_collection(net,
                                                       data_loaders,
                                                       num_epochs,
                                                       patience,
                                                       conv1_mask,
...
def main():
	...
	experiment_data = collect_prune_conv_data(net, [0.95, 0.9, 0.8, 0.5, 0.25])
